...
public final class OpenAiTranslator {
    ...
    public static String translate(String apiKey, String model, String vietnamese) throws Exception {
        String selectedModel = model == null || model.trim().isEmpty() ? "gpt-5-mini" : model.trim();

        JSONObject request = new JSONObject();
        request.put("model", selectedModel);
        request.put("store", false);
        // Reasoning tokens are included in max_output_tokens on GPT-5-family models.
        // 220 was sometimes exhausted before a visible translation was emitted.
        ...
        // Keep translation fast and leave output-token budget for the actual answer.
        ...
        if (status < 200 || status >= 300) {
            throw new IllegalStateException("OpenAI API 오류 " + status + ": " + extractError(body));
        }

        JSONObject response = new JSONObject(body);

        // Some client representations expose output_text directly; keep this for compatibility.
        String top = response.optString("output_text", "").trim();
        if (!top.isEmpty()) return top;

        // Raw Responses API JSON normally carries assistant text under output[].content[].
        ...
        if ("incomplete".equals(responseStatus) || incomplete != null) {
            String reason = incomplete == null ? "알 수 없음" : incomplete.optString("reason", "알 수 없음");
            throw new IllegalStateException("OpenAI 응답이 완료되지 않았습니다. 원인: " + reason);
        }

        throw new IllegalStateException("번역 응답에서 텍스트를 찾지 못했습니다. 응답 상태: " + (responseStatus.isEmpty() ? "unknown" : responseStatus));
    }
